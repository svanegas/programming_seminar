...
\begin{frame}[fragile]
	...
	In C++, ever if you have a simple variable or an array you have to initialize them.\\
...
\begin{frame}[fragile]
	\frametitle{Not initialize!}
	If you are reading a value or a set of values,is not necessary to initialize.\\
	\begin{block}{Reading values}
		\src{C++}{./src/notInitArray.cpp}
	\end{block}
\end{frame}
\end{subsection}
\begin{subsection}{Check the scoreboard.}
\begin{frame}[fragile]
	\frametitle{ScoreBoard!}
	If you are stuck and you don't know what exercise do, take a look to the scoreboard, the exercise with more solutions\\
	must be an easy one.\\
\end{frame}
\end{subsection}
...
\begin{frame}[fragile]
	\frametitle{Divide!}
	Can be useful that each team to divide the problems in 3 sections, each participant \\
	will try to find a easy problem in his section, when nobody can find more easy \\
	problems, is time to try to solve a problem together.\\
\end{frame}
\end{subsection}
\end{section}
